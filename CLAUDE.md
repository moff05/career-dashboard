...
- `discovered_jobs` — pending review queue from the board-scan (see "Job-board scan" below): company, title, type, location, url, description, posting_date, source, match_score, score_data, status (`pending` / `added` / `dismissed` / `discarded`), created_at. `(user_id, url)` is a unique index — doubles as the dedup record so a posting is never re-surfaced once seen, even after it's been dismissed or discarded.
...
└── api/
    ...
    ├── discovered/route.ts, discovered/[id]/route.ts, discovered/refresh/route.ts   # Board-scan review queue — see "Job-board scan" below
    └── cron/scan-boards/route.ts        # Weekly cron entry point (all users) — see "Job-board scan" below
...
├── scoreJobFit.ts      # The fit-scorecard rubric itself, extracted so jobs/[id]/analyze and the board-scan share one copy
├── boardCompanies.ts   # Resolves a company's career_url to a Greenhouse/Lever/Ashby/Workday board and fetches its postings
└── scanBoards.ts        # scanBoardsForUser() — the per-user scan/filter/score/stage logic, called by both the cron loop and a user's manual refresh
...
Greeting + Priorities strip, then two tabs — **Tracker** (stats + the job tracker table) and **Discovered** (the board-scan review queue, with a count badge when non-empty) — top to bottom, one page:
- **Greeting + Priorities** — `getPriorities(jobs)` computes free, no-AI-call priority items from whatever's already loaded: deadlines (≤3 days = urgent, ≤7 = soon), 14+ day no-response follow-ups, interview-prep flags. Sorted by urgency bucket first, then `match_score` descending as a tiebreaker. Clicking a priority item calls `jumpToJob(id)` — expands that job's row in the table below and scrolls to it (`id="job-row-{id}"`), rather than navigating anywhere. An "interview prep" priority instead opens the Coach overlay with a prefilled prompt about that job. This is the only "what should I prioritize" surface — Weekly Brief and the Strategy advisor were both cut as redundant with it (see roadmap). Sits above both tabs — visible regardless of which one is active.
...
- **Flat, sortable job list** (Sept 2026 — see roadmap) — no company-group accordion anymore. Each row shows its company inline (a kicker line above the title); click a row to expand its own detail panel. The two-level accordion (company group → job row) was cut because most tracked companies only have 1-2 roles, so the extra grouping level was friction without payoff. Sortable by Company, Title, Type, Status, Score, Deadline, Posted (click a column header, 3-click cycle: asc → desc → off). Default (no explicit sort): starred jobs float to the top, then most-recently-added.
- **Job type** condensed to `full-time` / `part-time` / `internship` (`TYPE_OPTIONS` in `app/lib/jobUtils.tsx`) — the old fall/spring/summer-internship season split (plus a `type_year` field) was cut as unnecessary once someone is only tracking full-time roles; the `type_year` DB column still exists but nothing writes to it anymore.
...
## Job-board scan (Discovered tab)

For any user with `target_roles` and `target_cities` set in Profile, a **weekly** cron (`vercel.json`, Mondays 13:00 UTC → `GET /api/cron/scan-boards`) scans that user's own tracked Companies list — whichever resolve to a Greenhouse/Lever/Ashby/Workday board via `companies.career_url` (`lib/boardCompanies.ts`) — filters postings by role/city match, scores survivors with the same fit rubric as a manual AI Score (`lib/scoreJobFit.ts`), and stages up to `TOP_N` (20) as `pending` rows in `discovered_jobs`. Nothing ever touches the real `jobs` table automatically — the user reviews each one from the Discovered tab and explicitly Adds or Dismisses it (`PATCH /api/discovered/[id]`).

The per-user scan/filter/score/stage logic lives in `lib/scanBoards.ts` (`scanBoardsForUser`), shared by two callers:
- The cron route loops every eligible user, sharing one `postingsCache` across them so two users tracking the same company don't double-fetch its board.
- **Manual refresh** (`POST /api/discovered/refresh`) lets a user pull a fresh batch without waiting for next Monday — but only once their current queue is fully cleared (zero `pending` rows). This is deliberate: approving or dismissing everything in the current batch is the signal that they've actually looked at it, which is what unlocks pulling the next one. Refreshing while unreviewed rows still exist would just bury them under a second batch, so the route 409s instead. The Discovered tab's Refresh button is disabled for the same reason whenever `discoveredJobs.length > 0`.

Each Discovered row expands (click anywhere on the row, not just the chevron) to show the same score-category breakdown as a tracked job's AI Score tab (summary + per-category bars, read from `d.score_data` — already computed at scan time, no extra AI call) plus the full parsed description in a collapsible block. `GET /api/discovered` also returns `lastScanAt` (`MAX(created_at)` across any status for that user) so the tab can show "Last scanned Xh/d ago."

**Safety history — read before touching the matching logic in `lib/scanBoards.ts` or the cron route.** An earlier version matched on single words extracted from `target_roles` ("engineer", "data", "analyst", ...) and inserted directly into the real tracker. Tested once against production, it matched almost every posting on a few large boards and wrote 1,030 junk rows into two real users' trackers before being caught and cleaned up. Two fixes came out of that and must not regress: (1) matching requires a bigram or exact phrase from `target_roles` — a single generic word is never enough; (2) nothing touches the real `jobs` table until the user explicitly approves a specific posting via the staging table. Use `?dryRun=1` on the cron route and/or a local sqlite DB to test any further change to the matching logic, not production.
...
- `POST /api/jobs/[id]/analyze` — 5-category fit scorecard, each category constrained to an enum of anchored values, weighted to a total out of 100: Explicit Requirements Met (0-25), Skills Match (0-25), Role Alignment (0-25), Industry Fit (0-10), Logistics/Location Fit (0-15). Industry Fit was cut from 20→10 (2026-09-02) — at 20% weight with zero credit for transferable skills, it was structurally close to unwinnable for an early-career candidate applying across industries; the freed 10 points went to Skills Match and Role Alignment instead. The Explicit Requirements rubric also explicitly credits any one satisfied path in an "X, Y, or Z" alternative-qualifying-paths requirement rather than only the first-listed one. `qwen/qwen3.6-27b` (Groq reasoning model), `temperature: 0`. A truncated response (this model can burn most of its completion budget on hidden reasoning before writing JSON) is detected server-side (missing category rationale/summary) and errored rather than silently defaulting a cut-off category to a score of 0. Scores *fit for this specific posting*, not general candidate strength — scored against what's literally on the resume today, with explicit-requirement misses capped low regardless of overall strength. Color coding on the total: 80+ green, 60–79 amber, <60 red; per-category bars are colored by percent-of-that-category's-max, not the raw score. This is the per-job AI score the whole tracker is built around.
...
27. **Feedback widget** (`app/components/FeedbackWidget.tsx`) — fixed bottom-right button opens a slide-up panel: name, message, optional screenshot attachment (2MB cap). `POST /api/feedback` stores to a `feedback` table in the DB (id, name, message, screenshot, user_agent, created_at). No AI triage — submissions are stored for manual review.
28. **Deleted orphaned legacy profile row** (`id=1`, `user_id=NULL`) — predated the multi-user migration, was unreachable but present in the DB. Cleaned up in the same commit as per-feature cost hints.
29. **Switched AI provider to Groq** — all routes now use Groq via the OpenAI-compatible SDK (`lib/gemini.ts`, name is legacy). `llama-3.3-70b-versatile` for generative routes, `qwen/qwen3-32b` for fit scoring. Server-side `GROQ_API_KEY` only — BYOK model dropped entirely. `lib/usage.ts` pricing table updated to Groq rates.
...
30. **Fixed stale post-Groq-migration docs** — `README.md` and `.env.local.example` still described the pre-migration bring-your-own-Anthropic-key model and file-based backup/restore, both long since replaced (Groq shared server key, account-key recovery via `/api/account-key/redeem`). `.env.local.example` didn't even mention the actually-required `GROQ_API_KEY`.
31. **Chrome extension privacy-link fix** — Chrome Web Store rejected the v1.2.0 submission for "Privacy policy URL is indirect." The policy was only linked from the website's own pages (`/welcome`, `/extension` footers) — the extension's popup itself had zero reference to it, and Chrome's policy requires disclosures reachable "within the Product's user interface" (the product under review is the extension, not the website). Added a persistent "Privacy Policy" link in the popup footer (visible in every state, opens `/privacy` directly), bumped to 1.2.1, rebuilt `jobs-extension.zip`, resubmitted. **Awaiting Chrome's re-review — not yet cleared.**
32. **Fit-scorecard rubric fix** (2026-09-02) — found by Nicholas comparing his own 2 real tracked jobs against his resume. Explicit Requirements Met was misreading a disjunctive requirement ("2+ years in an operating role, finance, consulting, or founding your own venture") as if only the first-listed path counted; now explicitly credits any one satisfied alternative. Industry Fit dropped 20→10 points (redistributed to Skills Match/Role Alignment, now 25 each) — at 20% weight with zero credit for transferable skills, it was his lowest-or-near-lowest category on both real jobs, structurally close to unwinnable for an early-career candidate applying across industries. Also caught and fixed a latent bug while verifying against the live Turso DB: a truncated model response (this reasoning model can burn most of its completion budget on hidden reasoning before writing any JSON) still parses as valid-but-incomplete JSON, and the route was silently defaulting a missing category to a score of 0 with no error shown — would have written a wrong, artificially low total to the DB. Now validated server-side and errored instead.
33. **UI/UX pass on scale** (2026-09-11) — driven by real usage complaints once Nicholas had ~20 tracked jobs, 38 companies, and 19 connections: the tracker's two-level accordion (company group → job row) had become tedious to navigate with mostly 1-role companies, and the Companies/Connections modals had no search or filter at all, just a long scroll of full-size cards. Flattened the job tracker to one sortable/filterable list (see "Flat, sortable job list" above) — dropped `expandedCompanies` state and the `Object.entries(grouped)` render entirely, added a Company column/sort, and fixed a real CSS grid blowout bug the change surfaced (a `1fr` track without `minmax(0, 1fr)` lets an unbreakable long title push every column after it out of alignment — caught via Playwright screenshot against live data, not just code review). Added a search box + status-filter pills to both `CompaniesPanel` and `ConnectionsPanel` (search matches name/company/role for connections, name only for companies), and tightened both panels' card padding for a denser scan. Condensed job `type` from the fall/spring/summer-internship split down to `full-time` / `part-time` / `internship` (see "Job type" above) — also fixed `POST /api/jobs/import`'s type-inference prompt, which was still asking the model for the pre-migration `fall-2026-internship`/`spring-2027-internship` string values from before Done #10's generic-type rename, meaning AI-imported jobs could get a type value the UI no longer recognized.
34. **Job-board scan shipped** (2026-09-12, commit f8f849b) — see "Job-board scan" above for the full mechanism and the matching-logic safety history. Scans every tracked company resolving to a Greenhouse/Lever/Ashby/Workday board, filters by the user's own target_roles/target_cities, scores matches with the existing fit rubric (extracted from `jobs/[id]/analyze` into `lib/scoreJobFit.ts` so both share one copy), and stages the best candidates into `discovered_jobs` for explicit review — nothing auto-adds to the real tracker. Originally ran daily; see #35 for the move to weekly.
35. **Discovered tab + weekly cadence + manual refresh gate** (2026-09-14) — the board-scan review queue had been a small inline block above the tracker with no way to see a posting's parsed details (clicking a row just opened the external link) and no way to pull a fresh batch outside the daily cron. Real-usage ask from Nicholas: move it somewhere better-organized, let him see what was actually parsed, and slow the cadence down since daily was noisier than needed — but let him pull an early batch himself once he's actually looked at what's there.
    - **Tabs** — Tracker/Discovered replace the old single-page layout, with a count badge on Discovered so a fresh batch is noticeable. Priorities stays above both tabs (applies to tracked jobs regardless of which tab is active).
    - **Expandable detail per row** — clicking a Discovered row (not just an icon) expands it in place to show the same score-category breakdown as a tracked job's AI Score tab (reusing `d.score_data`, already computed at scan time — no extra AI call) plus the full parsed description. Previously the only way to see anything was the external link.
    - **Weekly cadence** — `vercel.json`'s cron moved from daily (`0 13 * * *`) to Mondays only (`0 13 * * 1`). Also raised `TOP_N` 10→20 and `MAX_CANDIDATES_PER_USER` 20→40 in `lib/scanBoards.ts` (per Nicholas: same companies/matching logic, just don't let good matches get silently discarded when there's a week's backlog instead of a day's) — explicitly *not* the auto-discover-new-companies direction, which was ruled out as too close to the matching-logic incident described above.
    - **Manual refresh, gated on a cleared queue** — new `POST /api/discovered/refresh`, single-user version of the same `scanBoardsForUser()` the cron calls. 409s if any `pending` row still exists for that user; the Discovered tab's Refresh button is disabled for the same reason. This was the specific behavior Nicholas asked for: approving/dismissing everything in the current batch is what signals he's reviewed it, and that's what unlocks pulling the next one early instead of waiting for Monday.
    - Refactored the cron route itself down to a thin per-user loop — the actual scan/filter/score/stage logic moved to `lib/scanBoards.ts` (`scanBoardsForUser`) so the manual-refresh route could call the identical code path rather than duplicating it.
    - Verified against Nicholas's real account (19 tracked jobs, 10 pending discovered rows at the time): tab counts, row expansion showing category-by-category scores, and the refresh 409 gate all confirmed live, not just in code.
...
- **Discover page entirely** (`/discover`, Hunt Agent, Idea Feed, company research, `leads` table) — the Hunt Agent wasn't reliable and, more importantly, wasn't specific enough to the user's actual target roles/locations to be worth the agentic-search complexity. Company research was "cool but not necessary — they can ask Google." `lib/ats.ts` and `lib/liveness.ts` were deleted as dead code (only the Hunt Agent used them). **Not the same thing as the Discovered tab's job-board scan** (Done #34/#35) — that's deliberately narrower: only scans companies the user already tracks (no AI-driven company discovery), stages to a review queue instead of ever writing to the real tracker, and exists specifically because that scoped version tested safely where the old Hunt Agent didn't. Don't read this cut entry as covering the current feature.
...
### Not yet done
- **Chrome Web Store approval for the extension** — built and functional (see Done #31), resubmitted as v1.2.1 after the privacy-link rejection. Not live on the Store until Chrome clears re-review.
- **August 2026**: Stripe integration + per-user usage cap enforcement (~$5/month, token budgets, rate limiting before public launch).
