...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!company || !title || !type) {
      return NextResponse.json({ error: 'company, title, type are required' }, { status: 400 });
    }

    const result = await db.execute({
      sql: `INSERT INTO jobs (company, title, type, status, match_score, posting_date, deadline, url, description, salary_range, location, source, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        company, title, type, status || 'saved',
