    ...
    return (
      <div style={{ marginBottom: '14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <span style={{ color: '#628aaa', fontSize: '10px', display: 'block', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>{label}</span>
          {!isEditing && (
            <button onClick={() => startEdit(field, value || '')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3c5875', padding: '0 2px' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#8eb0cc')}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {[
            ...
            <div key={section.title} style={cardStyle}>
              <h3 style={{ color: '#628aaa', fontSize: '10px', fontWeight: 700, margin: '0 0 16px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{section.title}</h3>
        ...
        <div style={cardStyle}>
          ...
          {!summary && !loadingSummary && (
            <div style={{ textAlign: 'center', padding: '48px 24px' }}>
              <p style={{ color: '#507090', fontSize: '13px', margin: '0 0 8px' }}>Click &quot;Generate&quot; for an AI assessment of your candidacy.</p>
              <p style={{ color: '#3c5875', fontSize: '12px', margin: 0 }}>Analyzes your resume, experiences, and saved context.</p>
            </div>
          )}
          ...
          {summary && !loadingSummary && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px', padding: '14px 16px', backgroundColor: '#0e1520', borderRadius: '12px' }}>
                ...
                <div>
                  <div style={{ color: '#eaf2ff', fontWeight: 700, fontSize: '13px' }}>Readiness Score</div>
                  <div style={{ color: '#628aaa', fontSize: '12px', marginTop: '3px' }}>For competitive tech/CRE roles</div>
                </div>
              </div>
              <p style={{ color: '#d5e5f5', fontSize: '13px', lineHeight: '1.65', margin: '0 0 16px' }}>{summary.summary}</p>
              <div style={{ marginBottom: '14px' }}>
                ...
                <ul style={{ margin: 0, paddingLeft: '16px' }}>
                  {summary.strengths.map((s, i) => <li key={i} style={{ color: '#d5e5f5', fontSize: '13px', marginBottom: '5px' }}>{s}</li>)}
                </ul>
              </div>
              <div>
                ...
                <ul style={{ margin: 0, paddingLeft: '16px' }}>
                  {summary.gaps.map((g, i) => <li key={i} style={{ color: '#d5e5f5', fontSize: '13px', marginBottom: '5px' }}>{g}</li>)}
                </ul>
              </div>
            </div>
          )}
